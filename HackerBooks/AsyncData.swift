...
class  AsyncData {
    ...
    convenience init(url: URL, defaultData : Data, imageView: UIImageView){
        self.init(url: url, defaultData: defaultData)
    ...
    func loadLocalData() -> Bool{
        ...
        if fm.fileExists(atPath: local.path){
            ...
            if self.image == nil{
                ...
                delegate?.asyncData(self, didEndLoadingFrom: local, withImage: image!)
            }
            sendNotification()
            return true
        }else{
            ...
            DispatchQueue.global(qos: .default).async {
                ...
                DispatchQueue.main.async {
                    ...
                    if self.image == nil{
                        ...
                        self.delegate?.asyncData(self, didEndLoadingFrom: self.url, withImage: self.image!)
                    }
